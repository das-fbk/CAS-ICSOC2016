...
public class Analyzer {
    ...
    public DefaultEdge resolveIssue(CollectiveAdaptationProblem cap, String capID, IssueResolution res,
	    EnsembleManager en, CAWindow window, CATree cat, List<EnsembleManager> ensembles,
	    CollectiveAdaptationSolution solution1, int issueIndex, CATree hierarchyTree,
	...
	case "ISSUE_RECEIVED": {
	    ...
	    lastEdge = manageIssueReceived(cap, capID, res, en, cat, window, ensembles, issueIndex, hierarchyTree,
		    graph, lastEdge);
	...
	case "ISSUE_TARGETED": {
		...
		for (ConcurrentHashMap.Entry<Solver, List<IssueCommunication>> solution : rm.getSolutions()
			...
			// System.out.println("WAITING SOLUTIONS FROM
			// PARTNERS");
		    }
	...
	case "COMMIT_REQUESTED": {
		...
		for (ConcurrentHashMap.Entry<Solver, List<IssueCommunication>> solution : rm.getSolutions()
			...
			cat.insertEdge(cat.getDefaultParent(), "", label2 + ": " + command, v1, v1, STYLE_ISSUE_EDGE);
			res.setStatus("End");
			leaf = true;
	...
	case "COMMIT_SENT": {
		...
		if (res.getCommunications().size() > 1) {
			...
			countTargets = 0;
			targets = res.getCommunications().get(i).getTargets().size();

			for (int j = 0; j < res.getCommunications().get(i).getTargets().size(); j++) {
			    Target t = res.getCommunications().get(i).getTargets().get(j);
		    ...
		    targets = res.getCommunications().get(0).getTargets().size();
	...
	case "ISSUE_TRIGGERED": {
	    ...
	    generateSubTree(cap, capID, res, en, cat, rm, is, roleName, window, ensembles, issueIndex, graph, lastEdge);
	    // showTree(graph);
...

	{
		...
		if (coms.size() > 1) {
		    ...
		    cat.insertEdge(cat.getDefaultParent(), "", "COMMIT_REQUESTED", currentNode, v3, STYLE_ISSUE_EDGE);

		    // update status of the target
		    if (rm1.getIssueResolutions().size() > 1) {

			IssueResolution issue = rm1.getIssueResolutions().get(1);
    ...
    private DefaultEdge manageIssueReceived(CollectiveAdaptationProblem cap, String capID, IssueResolution res,
	    EnsembleManager en, CATree cat, CAWindow window, List<EnsembleManager> ensembles, int issueIndex,
	    CATree hierarchyTree, DirectedGraph<String, DefaultEdge> graph, DefaultEdge lastEdge) {
	...
	lastEdge = ExecuteSolvers(cap, capID, res, en, cat, rm, res.getIssueInstance(), r.getType(), window,
    ...
    private DefaultEdge generateSubTree(CollectiveAdaptationProblem cap, String capID, IssueResolution res,
	    EnsembleManager en, CATree cat, RoleManager rm, Issue is, String roleName, CAWindow window,
	    List<EnsembleManager> ensembles, int issueIndex, DirectedGraph<String, DefaultEdge> graph,
	...
	ExecuteSolvers(cap, capID, res, en, cat, rm, is, roleName, window, currentNode, ensembles, issueIndex, graph,
		lastEdge);
	// showTree(graph);
	// System.out.println(roleName + " *****ISSUE_TRIGGERED*****");
	return lastEdge;
    ...
    private DefaultEdge ExecuteSolvers(CollectiveAdaptationProblem cap, String capID, IssueResolution res,
	    EnsembleManager en, CATree cat, RoleManager rm, Issue is, String roleName, CAWindow window,
	    Object currentNode, List<EnsembleManager> ensembles, int issueIndex,
	    DirectedGraph<String, DefaultEdge> graph, DefaultEdge lastEdge) {
	...
	if (solvers.size() == 1) {
	    ...
	    lastEdge = createSingleCom(capID, res, en, cat, currentNode, rm, roleName, window, solver, ensembles, graph,
		    lastEdge);
	    // showTree(graph);
	    // showCAT(cat);
	    ...
	    rm.setSolverInstances(solvers);
	    createMultipleCom(cap, capID, res, en, cat, currentNode, rm, roleName, window, ensembles, issueIndex, graph,
		    lastEdge);
    ...
    private DefaultEdge createMultipleCom(CollectiveAdaptationProblem cap, String capID, IssueResolution res,
	...
	String last = "";
	GraphIterator<String, DefaultEdge> iterator3 = new BreadthFirstIterator<String, DefaultEdge>(graph);
	...
	for (int i = 0; i < rm.getSolverInstances().size(); i++) {
		...
		targetWithIssues(res, en, cat, currentNode, rm, roleName, currentSolver, ensembles, graph, lastEdge);
    ...
    private DefaultEdge createSingleCom(String capID, IssueResolution res, EnsembleManager en, CATree cat,
	    Object currentNode, RoleManager rm, String roleName, CAWindow window, Solver solver,
	    List<EnsembleManager> ensembles, DirectedGraph<String, DefaultEdge> graph, DefaultEdge lastEdge) {

	// A Solver Exists
	res.setStatus("SOLVER_CALLED");

	// we create the subtree related to the unique solver identified
	createSolverTree(res, en, cat, currentNode, rm, roleName, solver, ensembles, graph, lastEdge);

	return lastEdge;
    }

    private DefaultEdge createSolverTree(IssueResolution res, EnsembleManager en, CATree cat, Object currentNode,
	...
	if (solver.getSolution().get(0).getIssue().size() > 0) {
	    ...
	    targetWithIssues(res, en, cat, currentNode, rm, roleName, solver, ensembles, graph, lastEdge);
	...
	createSubTree(en, cat, currentNode, rm, solver, communications, graph, lastEdge);
    ...
    private void createSubTree(EnsembleManager en, CATree cat, Object currentNode, RoleManager rm, Solver solver,
	    ArrayList<IssueCommunication> communications, DirectedGraph<String, DefaultEdge> graph,
	    DefaultEdge lastEdge) {
	String issues = "";
	...
	if (solver.getSolution().get(0).getIssue().size() > 1) {
	    ...
	    addMultipleTargets(en, cat, rm, solver, communications, v2, graph, lastEd);
	} else {
	    ...
	    // add the target role
	    addTargetNode(en, cat, currentNode, rm, communications, 0, solver, graph, e);
	    // showTree(graph);
    ...
    private DefaultEdge addMultipleTargets(EnsembleManager en, CATree cat, RoleManager rm, Solver solver,
	    ArrayList<IssueCommunication> communications, Object v2, DirectedGraph<String, DefaultEdge> graph,
	    DefaultEdge lastEdge) {
	Object currentNode;
	...
	for (int j = 0; j < communications.size(); j++) {
		...
		lastEdge = addTargetNode(en, cat, currentNode, rm, communications, j, solver, graph, lastEdge);
		// showTree(graph);
		// showCAT(cat);
    ...
    private DefaultEdge addTargetNode(EnsembleManager en, CATree cat, Object currentNode, RoleManager rm,
	...
	String index = "" + en.getCount();
	en.setCount(en.getCount() + 1);
	String ensembleName = currentTarget.getTargetRole().getEnsemble().getEnsemble().getName();
	...
	IssueResolution ir = new IssueResolution(1, "ISSUE_RECEIVED", rm, targetManager, currentCom.getIssueToSolve(),
		null);
	ir.setIssueResolutionID(en.getIssueResolutionCount());
	ir.setRoot(false);
