package allow.simulator.collective.adaptation.presentation;
...
import allow.simulator.collective.adaptation.RoleManager;
...
import allow.simulator.collective.adaptation.api.CollectiveAdaptationSolution;
import allow.simulator.collective.adaptation.api.RoleCommand;
import allow.simulator.collective.adaptation.ensemble.Ensemble;
import allow.simulator.collective.adaptation.ensemble.Issue;
import allow.simulator.collective.adaptation.model.IssueCommunication;
import allow.simulator.collective.adaptation.model.IssueResolution;
...
public class CAWindow extends JFrame {
    ...
    public CAWindow(List<EnsembleManager> ems, CollectiveAdaptationProblem cap, int IssueIndex,
	...
	btnRunCollectiveAdaptation.addMouseListener(new MouseAdapter() {
		...
		run(cap, ems, cap.getIssue(), cap.getCapID(), cap.getStartingRole(), IssueIndex);
		btnRunCollectiveAdaptation.setEnabled(false);
		btnApplySolution.setEnabled(true);

	    }
	});
	btnRunCollectiveAdaptation.setToolTipText("run collective adaptation algorithm");
	...
	btnApplySolution.setToolTipText("apply collective adaptation solution, one command at time");
	...
	IssueResolutionsList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	...
	graphComponentHierarchy.getGraphControl()
		.addMouseListener(new MouseTreeNodeListener(graphComponentHierarchy, this.ensm, this));

	graphComponentHierarchy.setBounds(600, 125, 650, 600);
	graphComponentHierarchy.setBorder(javax.swing.BorderFactory.createLineBorder(new Color(0, 0, 0)));
	...
	PossibleSolutionList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	...
	logger.info("Applying solution for collective problem with id " + cas.getCapID());
	// for each ensemble
    ...
    public void run(CollectiveAdaptationProblem cap, List<EnsembleManager> ensembles, String issueName, String capID,
	...
	for (int i = 0; i < ensembles.size(); i++) {
		...
		if (currentManager.getRole().getType().equalsIgnoreCase(r.getRole().getType())) {
		    ...
		    em.checkIssues(cap, capID, this, ensembles, solution, issueIndex, hierarchyTree);
