...
public class Utilities {
    ...
    public static void setSolversMapDrones(HashMap<String, ArrayList<RoleManager>> solversMap) {
    ...
    public static void setSolversMapMobility(HashMap<String, ArrayList<RoleManager>> solversMap) {
	...
	int numElements = Utilities.solversMapMobility.get(issue.getIssueType()).size();
	if (numElements == 1) {
	    result = Utilities.solversMapMobility.get(issue.getIssueType()).get(0);
	}
	if (numElements > 1) {
	    int index = Utilities.generateRandomValues(numElements - 1, 1).get(0);
	    result = Utilities.solversMapMobility.get(issue.getIssueType()).get(index);
	}
	return result;
    }
	...
	int numElements = Utilities.solversMapDrones.get(issue.getIssueType()).size();
	if (numElements == 1) {
	    result = Utilities.solversMapDrones.get(issue.getIssueType()).get(0);
	}
	if (numElements > 1) {
	    int index = Utilities.generateRandomValues(numElements - 1, 1).get(0);
	    result = Utilities.solversMapDrones.get(issue.getIssueType()).get(index);
	}
	return result;
    }
	...
	Utilities.solversMapMobility.put("IntenseTraffic", new ArrayList<RoleManager>());
	Utilities.solversMapMobility.put("RouteBlocked", new ArrayList<RoleManager>());
	Utilities.solversMapMobility.put("PassengerDelay", new ArrayList<RoleManager>());
	Utilities.solversMapMobility.put("DriversStrike", new ArrayList<RoleManager>());
	Utilities.solversMapMobility.put("CPAPassengerDelay", new ArrayList<RoleManager>());
	...
	Utilities.solversMapDrones.put("Drone1Fault", new ArrayList<RoleManager>());
	Utilities.solversMapDrones.put("Drone3Fault", new ArrayList<RoleManager>());
	Utilities.solversMapDrones.put("IntruderDetected", new ArrayList<RoleManager>());
	Utilities.solversMapDrones.put("ObstacleFound", new ArrayList<RoleManager>());
	Utilities.solversMapDrones.put("CameraFault", new ArrayList<RoleManager>());
    ...
    private static void assignStaticallyRolesMobility(HashMap<String, String[]> staticMap,
	    List<RoleManager> roleManagers) {
	Iterator iterator = staticMap.entrySet().iterator();
	while (iterator.hasNext()) {
	    Map.Entry pair = (Map.Entry) iterator.next();
	    Utilities.solversMapMobility.put((String) pair.getKey(),
		    Utilities.getManagersByIssueNames((String[]) pair.getValue(), roleManagers));
	}
    }

    private static void assignStaticallyRolesDrones(HashMap<String, String[]> staticMap,
	    List<RoleManager> roleManagers) {
	Iterator iterator = staticMap.entrySet().iterator();
	while (iterator.hasNext()) {
	    Map.Entry pair = (Map.Entry) iterator.next();
	    Utilities.solversMapDrones.put((String) pair.getKey(),
		    Utilities.getManagersByIssueNames((String[]) pair.getValue(), roleManagers));
	}
    }
